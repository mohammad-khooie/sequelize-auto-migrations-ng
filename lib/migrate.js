...
function reverseSequelizeColType(col, prefix = 'Sequelize.') {
  ...
  // Sequelize.CHAR
  if (attrObj.constructor.name === "CHAR") {
  ...
  // Sequelize.STRING
  if (attrObj.constructor.name === "STRING") {
  ...
  // Sequelize.TEXT
  if (attrObj.constructor.name === "TEXT") {
  ...
  // Sequelize.DOUBLE
  if (attrObj.constructor.name === "DOUBLE") {
  ...
  if (attrObj.constructor.name === "INTEGER" || attrObj.constructor.name === "FLOAT" || attrObj.constructor.name === "REAL" || attrObj.constructor.name === "DOUBLE") {
    let ret = attrObj.key;
    if (options.length) {
      ret += `(${options.length}`;
  ...
  if (attrObj.constructor.name === "ARRAY") { return `${prefix}ARRAY(${reverseSequelizeColType(attrObj, prefix)})`; }

  // Sequelize.ENUM
  if (attrObj.constructor.name === "ENUM") { return `${prefix}ENUM('${options.values.join("', '")}')`; }

  // Simple types
  if (attrObj.constructor.name === "BOOLEAN") { return `${prefix}BOOLEAN`; }

  if (attrObj.constructor.name === "TIME") { return `${prefix}TIME`; }

  if (attrObj.constructor.name === "DATEONLY") { return `${prefix}DATEONLY`; }

  if (attrObj.constructor.name === "DATE") { return `${prefix}DATE`; }

  // Not documented, really?
  if (attrObj.constructor.name === "HSTORE") { return `${prefix}HSTORE`; }

  if (attrObj.constructor.name === "JSONB") { return `${prefix}JSONB`; }

  if (attrObj.constructor.name === "JSON") { return `${prefix}JSON`; }

  if (attrObj.constructor.name === "UUID") { return `${prefix}UUID`; }

  // Virtual data type, we must to skip it
  if (attrObj.constructor.name === "VIRTUAL") { return `${prefix}VIRTUAL`; }
  ...
  for (const model in models) {
    ...
    for (const column in attributes) {
      ...
      for (const property in attributes[column]) {
        ...
        if (property === 'unique') { attributes[column][property] = true }

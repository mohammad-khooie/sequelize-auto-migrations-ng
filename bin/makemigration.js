                                ...
                                .then( () => {
                                    queryInterface.bulkInsert( "SequelizeMetaMigrations", rows )
                                        .then( () => {
                                            ...
                                            if ( options.execute ) {
                                                console.log( `Use sequelize CLI:sequelize db:migrate --to ${currentState.revision}-${info.info.name} ${options[ "migrations-path" ] ? `--migrations-path=${options[ "migrations-path" ]}` : ""} ${options[ "models-path" ] ? `--models-path=${options[ "models-path" ]}` : ""}` );
                                                process.exit( 0 );
                                            } else {
                                                process.exit( 0 );
                                            }

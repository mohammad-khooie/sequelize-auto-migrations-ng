...
function reverseSequelizeColType( col, prefix = "Sequelize." ) {
    ...
    // Sequelize.ENUM
    if ( attrObj.constructor.name === "ENUM" ) {
    ...
    if ( attrObj.constructor.name === "DATEONLY" ) {
        return `${prefix}DATEONLY`;
    }

    if ( attrObj.constructor.name === "DATE" ) {
        return `${prefix}DATE`;
    }
    ...
    if ( attrObj.constructor.name === "VIRTUAL" ) {
    ...
    for ( const model in models ) {
        ...
        for ( const column in attributes ) {
            ...
            for ( const property in attributes[ column ] ) {
                ...
                if ( property === "unique" ) {
                    attributes[ column ][ property ] = true;
                }
            ...
            case "addIndex":
                {
                    ...
                    const nameOrAttrs = ( action.options && action.options.name && action.options.name !== "" ) ? `"${action.options.name}"` : JSON.stringify( action.fields );
    ...
    return new Promise( ( resolve ) => {
        if ( options[ "keep-files" ] ) {
            ...
            fs.readdir( migrationsDir, ( err, files ) => {
                if ( err ) {
                    if ( options.debug ) {
                        console.error( `Can't read dir: ${err}` );
                    }
                    resolve( false );
